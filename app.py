...
# ══════════════════════════════════════════════════════════════════════════
# ★ WORKING MODELS — tested March 2026 ★
# ══════════════════════════════════════════════════════════════════════════
MODEL_PARSE = 'llama-3.3-70b-versatile'
MODEL_GRADE = 'llama-3.1-8b-instant'
...
# ═══════════════════════════════════════════════════════════════════════════
# MONGODB — pymongo with gevent safe settings
# ��══════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        if not ROLL_REGEX.match(roll_number):
            return jsonify({'error':
                'Roll number must be exactly 11 digits'}), 400
        if not password or len(password) < 6:
            return jsonify({'error':
                'Password must be at least 6 characters'}), 400
        if users_col.find_one(
                {'roll_number': roll_number, 'role': 'student'}):
            return jsonify({'error':
                f'Roll number {roll_number} already registered'}), 409
        ...
        print(f"[REGISTER] ✅ Student: {roll_number} — {name}")
        return jsonify({
...
def register_faculty():
    try:
        ...
        if secret_key != FACULTY_REG_KEY:
            return jsonify({'error':
                'Invalid registration key. Contact Admin / HOD.'}), 403
        ...
        if not re.match(r'^[a-zA-Z0-9_.]+$', username):
            return jsonify({'error':
                'Username: only letters, numbers, dot, underscore'}), 400
        ...
        if users_col.find_one(
            ...
            return jsonify({'error':
                f'Username "{username}" already taken'}), 409
        if email and users_col.find_one({'email': email}):
            return jsonify(
                {'error': 'Email already registered'}), 409
        ...
        users_col.insert_one(doc)
        print(f"[REGISTER] ✅ Faculty: {username} — {name}")
        return jsonify({
            'message': f'Faculty account created! Welcome {name}',
            'user_id': doc['user_id']
        }), 201
    except Exception as e:
        traceback.print_exc()
...
def api_login():
    try:
        ...
        if role == 'student':
            ...
            if not user:
                return jsonify({'error':
                    'Student not found. Please register first.'}), 404
        else:
            ...
            if not user:
                return jsonify({'error':
                    'Faculty not found. Please register first.'}), 404

        if not user.get('is_active', True):
            return jsonify({'error':
                'Account deactivated. Contact Admin.'}), 403

        stored = user.get('password', '')
        if not stored:
            return jsonify(
                {'error': 'No password set'}), 500
...
def api_logout():
    ...
    resp.delete_cookie(
        app.config['SESSION_COOKIE_NAME'], path='/')
    return resp
...
def create_exam():
    try:
        ...
        doc = {
            ...
            'pass_marks':      int(data.get('pass_marks', 40)),
            ...
            'status':          'scheduled',
            'randomize_q':     bool(data.get('randomize_q', True)),
            'randomize_o':     bool(data.get('randomize_o', True)),
            'violation_limit': int(data.get('violation_limit', 3)),
            'created_at':      utcnow()
        }
...
def stop_exam(exam_id):
    ...
    if not exam:
        return jsonify({'error': 'Exam not found'}), 404
    exams_col.update_one({'exam_id': exam_id},
                         {'$set': {'status': 'ended'}})
...
def ai_parse_questions(text, subject='General'):
    ...
    prompt = f"""You are an expert university exam question extractor.

Subject: {subject}

Extract ALL questions from the text below.
For EVERY question write a detailed model answer.
...
[
  ...
  {{
    "text": "Subjective question text",
    "type": "subjective",
    "marks": 5,
    "options": [],
    "correct_option": "",
    "model_answer": "Complete detailed model answer..."
  }}
]

Rules:
- type = "mcq" if options exist, else "subjective"
- options = list WITHOUT A) a. (A) prefix — ONLY the text
- correct_option = exact text of correct option
- model_answer = detailed answer for ALL questions
- marks: 1 MCQ, 2-5 short, 5-10 long answer
- Do NOT skip any question
- Options may be A) B) or a. b. or (A) (B) — strip the prefix
...
JSON array only:"""
    ...
    try:
        ...
        if start == -1 or end <= start:
            print("[AI Parse] No JSON array found")
            return None
...
def ai_generate_model_answers(questions, subject='General'):
    client_ai = get_groq_client()
    if not client_ai:
        return questions

    need = [(i, q) for i, q in enumerate(questions)
            if not q.get('model_answer', '').strip()]
    if not need:
        return questions

    print(f"[AI Answers] Generating for "
          f"{len(need)} questions")

    for batch_start in range(0, len(need), 5):
        batch = need[batch_start:batch_start + 5]
        q_text = '\n\n'.join([
            f"Q{idx+1} ({q['type']}, "
        ...
        prompt = f"""Expert {subject} professor.
Write detailed model answers.

{q_text}

Return ONLY JSON array:
[{{"answer": "Complete answer for Q1..."}},
 {{"answer": "Complete answer for Q2..."}}]"""
...
def upload_file(exam_id):
    ...
    try:
        ...
        if fname.endswith('.pdf'):
            ...
            if PYMUPDF_OK:
                ...
                doc.close()
                print(f"[PDF] fitz: {len(text)} chars")
            elif PYPDF_OK:
        ...
        # ── TXT ──────────────────────────────────────
            ...
            return jsonify({'error':
                'Only PDF, DOCX, TXT supported'}), 400
    ...
    if not text or len(text) < 15:
        return jsonify({'error':
            'No readable text found in file'}), 400
...
def get_questions(exam_id):
    ...
    qs = list(questions_col.find(
        {'exam_id': exam_id}).sort('index', ASCENDING))
    if session['role'] == 'student':
        try:
            seed = (int(session['user_id'][:8], 16)
...
def auto_grade(exam_id, user_id):
    ...
    print(f"[AutoGrade] {obtained}/{total} "
          f"user={user_id[:8]}")
    return obtained
...
def get_submissions(exam_id):
    ...
    for s in subs:
        ...
        u   = (users_col.find_one({'_id': oid})
               if oid else None)
        s['student_name'] = (u.get('name', 'N/A')
                             if u else 'N/A')
        s['roll_number']  = (u.get('roll_number', '')
                             if u else '')
        out.append(s)
    return jsonify(out)
...
def publish_results(exam_id):
    ...
    for i, r in enumerate(rows):
        rank       = i + 1
        percentile = (round((1 - (rank-1)/n)*100, 2)
                      if n else 0)
...
def get_results():
    ...
    for d in docs:
        ...
        if eid not in exam_cache:
            exam_cache[eid] = exams_col.find_one(
                {'exam_id': eid})
        exam = exam_cache[eid]
        d['exam_title']    = (exam.get('title', eid)
                              if exam else eid)
        d['exam_subject']  = (exam.get('subject', '')
                              if exam else '')
        d['total_marks']   = (exam.get('total_marks', 0)
                              if exam else 0)
        d['pass_marks']    = (exam.get('pass_marks', 0)
                              if exam else 0)
        d['duration_mins'] = (exam.get('duration_mins', 0)
                              if exam else 0)
        out.append(d)

    out.sort(key=lambda x: x.get('published_at', ''),
             reverse=True)
    return jsonify(out)
...
def get_exam_results(exam_id):
    ...
    for d in docs:
        ...
        d['name']          = (u.get('name', '')
                              if u else '')
        d['roll_number']   = (u.get('roll_number', '')
                              if u else '')
        d['exam_title']    = (exam.get('title', exam_id)
                              if exam else exam_id)
        d['exam_subject']  = (exam.get('subject', '')
                              if exam else '')
        d['total_marks']   = (exam.get('total_marks', 0)
                              if exam else 0)
...
def analytics(exam_id):
    ...
    passed    = sum(1 for r in rdocs if r.get('pass'))
...
def ai_grade(question, answer, marks,
    ...
    if client_ai:
        try:
            prompt = f"""You are a strict examiner for {subject}.
...
{{
  ...
  "key_points": ["<point>", "<point>"]
            ...
            return {
                ...
                'model':      f'groq/{MODEL_GRADE}'
            }
        except Exception as e:
            print(f"[AI Grade] Error: {e}")

    # Keyword fallback
    stops = {'this', 'that', 'with', 'from', 'have',
             'will', 'what', 'when', 'where', 'which',
             'they', 'their', 'about', 'would',
             'could', 'should'}
    qw    = set(re.findall(r'\b[a-z]{4,}\b',
                           question.lower())) - stops
    aw    = set(re.findall(r'\b[a-z]{4,}\b',
...
@app.route('/api/exams/<exam_id>/ai-grade/<user_id>',
           methods=['POST'])
@role_required('faculty')
def ai_grade_all(exam_id, user_id):
    try:
        ...
        print(f"[AI] {len(results)} graded, "
              f"{skipped} MCQ skipped, "
              f"total={total_ai}")

        return jsonify({
            'message':          f'AI graded {len(results)} questions',
...
@app.route('/api/exams/<exam_id>/ai-results/<user_id>',
           methods=['GET'])
@role_required('faculty')
...
def admin_list_users():
    ...
    for u in users:
        ...
        for f in ['created_at', 'last_login']:
            if isinstance(
                    u.get(f), datetime.datetime):
                u[f] = u[f].isoformat()
    return jsonify(users)


@app.route('/api/admin/users/<user_id>/toggle',
           methods=['POST'])
...
def admin_reset_password():
    ...
    if not roll or not new_pass:
        return jsonify({'error':
            'roll_number and new_password required'}), 400
    hashed = bcrypt.generate_password_hash(
...
def print_banner():
    ...
    print(f'  Parse Model : {MODEL_PARSE}')
    print(f'  Grade Model : {MODEL_GRADE}')
